...
#用來顯示預訂名單,過濾的條件為is_canceled.is_(False)為False
...
#透過get_audience_group_list來取得預訂過的受眾
def create_audience_group(event):

    audience_group_list = line_bot_api.get_audience_group_list(description='預訂過婚禮用酒', create_route='MESSAGING_API')
    ...
    massage_reservations = Reservation.query.filter(Reservation.booking_service_category == '婚禮').all()

    audiences = []

    for r in massage_reservations:
        audiences.append(r.user.line_id)#將所有有預訂的line_id加入audiences串列中

    audiences = [{'id': line_id} for line_id in list(set(audiences))]#整理成dict的型態

    line_bot_api.create_audience_group('預訂過婚禮用酒', audiences=audiences)
